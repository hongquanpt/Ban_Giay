...
namespace QuanLyBanGiay_ADMIN.Controllers
{
    public class SanPhamController : Controller
    {
        ...
        public IActionResult giao(string ma)
        {
            var dh = context.Hoadons.Where(x => x.MaHoadon == ma).FirstOrDefault();
            dh.TrangthaiHD = 2;
            context.SaveChanges();
            return RedirectToAction("ViewBill", "Checkout");
        }
...
*/
        ...
        [HttpGet]
        public IActionResult UpDateHinhAnh(string MaSanpham)
        {
            var da = context.HienThiYeuCaus.FromSqlRaw("YeuCauKH").ToList();
            ViewBag.YC = da.ToList();
            var sp_anh = context.AnhSps.Find(MaSanpham);
            if (sp_anh != null)
            {
                var sp = new AnhSp()
                {
        ...
        [HttpPost]
        public IActionResult UpDateHinhAnh(AnhSp New_Pic)
        {
            var da = context.HienThiYeuCaus.FromSqlRaw("YeuCauKH").ToList();
            ViewBag.YC = da.ToList();
            var dep = context.AnhSps.Find(New_Pic.MaSanpham);
            var temp = new AnhSp();
            if (dep != null) // Kiểm tra xem dep có khác null không
            {
            ...
            {
                temp.MaSanpham = New_Pic.MaSanpham;
                temp.Link1 = New_Pic.Link1;
                temp.Link2 = New_Pic.Link2;
                temp.Link3 = New_Pic.Link3;
                context.AnhSps.Add(temp);
                context.SaveChanges();
                return RedirectToAction("Details", "SanPham", new { MaSanpham = temp.MaSanpham });
            }
        }
...
*/
        ...
        public IActionResult HoaDon_ChuaXuLy(int page = 1)
        {
            ...
            page = page < 1 ? 1 : page;
            int pagesize = 4;
            var data = context.HienThiHoaDon_cxls.FromSqlRaw("HD_ChuaXuLy").ToList().ToPagedList(page, pagesize);
            return View(data);
        }

        public async Task<IActionResult> HuyHoaDon(string Ma_Hoadon)
        {
            var da = context.HienThiYeuCaus.FromSqlRaw("YeuCauKH").ToList();
            ViewBag.YC = da.ToList();
            ...
            //}
            //return RedirectToAction("Index");
            await context.Database.ExecuteSqlRawAsync("EXEC Huy_HoaDon {0},{1}", Ma_Hoadon);
            await context.SaveChangesAsync();
        ...
        public IActionResult ChiTietHoaDon_ChuaXuLy(string MaHoadon)
        {
            ...
            var data1 = context.HienThiHoaDon_cxls.FromSqlRaw("HD_ChuaXuLy").ToList();
            List<HienThiHoaDon_cxl> hd = new List<HienThiHoaDon_cxl>();
            hd = data1;
