	...
	public class CheckoutController : Controller
	{
		...
		public IActionResult AddCommentToDB(string masanpham, string comment, int numstar, int makhachhang)
		{
			if (HttpContext.Session.GetString("Username") is not null)
			{
				...
				_context.SaveChanges();
				return Json(new { status = "Đánh giá của bạn đã được lưu thành công !" });
			}
			return RedirectToAction("Index", "PageNotFound");
		...
		public async Task<IActionResult> YeuCauHuy(string Ma_Hoadon, string LyDo, string ThoiGian, int TrangThaiYeuCau, int TrangThaiXuLy)
		{
			...
			if(LyDo != "")
			{
				...
				yc.Ma_Hoadon = Ma_Hoadon;
				yc.ThoiGian = DateTime.ParseExact(ThoiGian, format, CultureInfo.InvariantCulture);
				yc.LyDo = LyDo;
				yc.TrangThaiYeuCau = 0;
				yc.TrangThaiXuLy = 0;

				_context.Database.ExecuteSqlRaw("Them_YC {0},{1},{2},{3},{4}", yc.Ma_Hoadon, yc.LyDo, yc.ThoiGian, yc.TrangThaiYeuCau, yc.TrangThaiXuLy);
				await _context.SaveChangesAsync();
				await Response.WriteAsync("<script>alert('Request is sent !!');window.location.assign('../Checkout/ViewBill');</script>");
